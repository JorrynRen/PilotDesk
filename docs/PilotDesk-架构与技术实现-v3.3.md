...
```rust
static LAST_DETECT: Lazy<Mutex<DetectCache>> = Lazy::new(|| {
    Mutex::new(DetectCache { value: None, fetched: None })
});
...
所有安装函数通过 `cmd /C` 执行命令，使用 `tauri::Emitter` 发送进度事件：

```rust
fn install_claude_code(app: tauri::AppHandle) -> Result<(), AppError> {
    // Command: cmd /C npm install -g @anthropic-ai/claude-code
    // 成功后 emit "install-progress" 事件
    // 清除 ENV_CACHE
}

fn install_hermes(app: tauri::AppHandle) -> Result<(), AppError> {
    // Command: cmd /C pip install hermes-agent
    // 成功后 emit "install-progress" 事件
    // 清除 ENV_CACHE
}

fn install_codex(app: tauri::AppHandle) -> Result<(), AppError> {
...
2. `Command::new("F:\\soft\\nodejs\\node.exe")` -- 使用固定 Node.js 路径
...
.setup(|app| {
    ...
    match sidecar.start(app_handle) {
        ...
        Err(e) => println!("[Sidecar] Failed to start: {}", e),
    }
...
**前端 Message 类型**（扩展字段，仅前端使用，尚未持久化到 SQLite）：
...
interface Message {
  ...
  timestamp: number;
  /** 推理/思考内容（如 DeepSeek reasoning_content），仅前端内存 */
  reasoningContent?: string;
  /** 模型请求的工具调用（JSON 数组字符串），仅前端内存 */
  toolCalls?: string;
  /** 工具调用 ID（role='tool' 时使用），仅前端内存 */
  toolCallId?: string;
  /** 工具名称（role='tool' 时使用），仅前端内存 */
  toolName?: string;
}
```

> 注：`reasoningContent`、`toolCalls`、`toolCallId`、`toolName` 目前仅在前端内存中使用，尚未持久化到 SQLite messages 表。后续可通过迁移添加对应列。
...
odejs
ode.exe --version
  |     +-- 回退: node --version (PATH)
  +-- git --version
  |     +-- 优先: Program Files\Gitin\git.exe
  |     +-- 回退: git --version (PATH)
  +-- python --version
  |     +-- 优先: LocalAppData\Programs\Python\Python3xx\python.exe
  |     +-- 回退: python / python3 (PATH)
  +-- claude --version
  |     +-- 优先: AppData
pm\claude.cmd
  |     +-- 回退: claude (PATH)
  ...
  +-- hermes --version
  |     +-- 优先: LocalAppData\hermes\hermes-agentenv\Scripts\hermes.exe
  |     +-- 次选: LocalAppData\Programs\Python\Python3xx\Scripts\hermes.bat
  |     +-- 回退: hermes (PATH)
  |     +-- clean_version_string 提取版本号
  +-- codex --version
        +-- 优先: AppData
...
install_agent(agentType)
  +-- claude -> cmd /C npm install -g @anthropic-ai/claude-code
  |     +-- 成功后 emit "install-progress" 事件
  |     +-- 清除 ENV_CACHE
  +-- hermes -> cmd /C pip install hermes-agent
  |     +-- 成功后 emit "install-progress" 事件
  |     +-- 清除 ENV_CACHE
  +-- codex  -> cmd /C npm install -g @openai/codex
        +-- 成功后 emit "install-progress" 事件
        +-- 清除 ENV_CACHE

update_agent(agentType)
  +-- claude -> npm update -g @anthropic-ai/claude-code
  +-- hermes -> pip install --upgrade hermes-agent
...
| `src-tauri/src/utils/paths.rs` | ~10 | 路径工具（app_data_dir, db_path） |
...
> PilotDesk 架构与技术实现 v3.3 | 2026-06-15
