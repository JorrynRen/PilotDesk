...
/* ─── Inspiration Panel (embedded in right sidebar) ─── */
...
function InspirationPanel() {
  ...
  const handleSearch = useCallback((query: string) => {
    ...
    debounceRef.current = setTimeout(() => {
      if (query.trim()) {
        // trigger a server-side search if store supports it
        // otherwise local filter is sufficient
      }
    }, 300);
  }, []);
    ...
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="shrink-0 px-3 py-2 space-y-2" style={{ borderBottom: '1px solid var(--border)' }}>
        <div className="flex items-center gap-1.5">
          ...
          {/* New button */}
          <button
            onClick={() => { setEditingInspiration(null); setShowForm(true); }}
      ...
      {showForm && (
        <InspirationInlineForm
          ...
          onCancel={() => { setShowForm(false); setEditingInspiration(null); }}
        />
      )}
        ...
        {loading && !inspirations.length ? (
          ...
          filteredInspirations.map((insp) => (
            <InspirationRow
              ...
              onEdit={(insp) => { setEditingInspiration(insp); setShowForm(true); }}
            />
          ))
        )}
      </div>
